...
public abstract class Tower : MonoBehaviour
{
    ...
    void FindEnemy()
    {
        ...
        for (int i = 0; i < hitColliders.Length; i++)
        {
            ...
            {
                if (hitColliders[i].gameObject.GetComponent<EnemytestScript>().lifeTime < currentAge)
                {
                    currentAge = hitColliders[i].gameObject.GetComponent<EnemytestScript>().lifeTime;
