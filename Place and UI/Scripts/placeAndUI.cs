...
public class placeAndUI : MonoBehaviour
{
    ...
    void Start()
    {
        if (tower.Length != texture.Length && tower.Length < 10)             //Test for basic value conditions
        {
            Debug.LogError("ERROR: Tower and Texture Array must have same size AND lower 10");
        ...
        if (Physics.Raycast(cam.transform.position, cam.transform.forward, out RaycastHit hit, placeRange, LayerM))
        {
            if (buildMode == true)                                //Activated buildMode
            {
                if (objectSelected == false)                      //actualisation if sth changed: Keyvalue, Visibility, buildMode                           
                {
                    ...
                    if (hit.transform.tag == "Ground" || hit.transform.tag == "Tower" || hit.transform.tag == "Wall")
                    {
                        towerObject = Instantiate(tower[keyValue - 1], hit.point, Quaternion.Euler(0, 0, 0));
                        towerObject.GetComponent<Renderer>().material = trueMaterial;    //set new Material to object
                        towerObject.tag = "Build";               //Tower get new Tag to know its in build mode
                        towerObject.layer = 11;                  //Layer set to 11: Build Layer
                ...
                else
                {
                    ...
                    if(towerObject.GetComponent<towerPlaceScript>().onlyGroundCollision == true)  
                    {
                        towerObject.GetComponent<Renderer>().material = trueMaterial;    //set new Material to object
                    ...
                    {
                        towerObject.GetComponent<Renderer>().material = falseMaterial;   //set new Material to object
                    }
                }
            }
            else
